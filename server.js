    ...
    socket.on('join_room', (payload) => {
        ...
        io.in(room).fetchSockets().then((sockets) => {
            ...
            if ((typeof sockets == 'undefined') || (sockets === null) || !sockets.includes(socket)) {
                response = {};
                response.result = 'fail';
                response.message = 'Server internal error joining chat toom.';
                socket.emit('join_room_response', response);
                serverLog('join_room command failed', JSON.stringify(response));
            }
    ...
    */
    ...
    socket.on('play_token', (payload) => {
        ...
        if ((typeof username == 'undefined') || (username === null)) {
            response = {};
            response.result = 'fail';
            response.message = 'play_token comand did not come from a registered username';
            socket.emit('play_token_response', response);
            serverLog('play_token command failed', JSON.stringify(response));
            return;
        }
...
function send_game_update(socket, game_id, message) {
    ...
    if ((typeof games[game_id] == 'undefined') || (games[game_id] === null)) {
        console.log("No game exists with game_id:"+ game_id +". Make a new game for " + socket.id);
        ...
        if(sockets.size >= 1) {
            ...
            if((games[game_id].player_white.socket != first) &&
               (games[game_id].player_black.socket != first)) {
                /* Then we know the play does not have a color yet */
                if (games[game_id].player_white.socket === "") {
                    /* then this player should be white */
                    ...
                    /* This player should be kicked out because they're a third player */
